import { BookOpen, Calculator, CheckCircle2, Clipboard, Download, Droplets, Leaf, Mountain, Ruler, Sprout, Zap } from "lucide-react";
...
function ResultSpotlight({ result, k, slopeLength, slopePercent, cp }: { result: ReturnType<typeof calculateEups>; k: number | null; slopeLength: number | null; slopePercent: number | null; cp: number | null }) { const factors: Array<{ icon: ReactNode; label: string; value: string; unit?: string }> = [{ icon: <Droplets />, label: "Precipitação anual", value: formatNumber(result.precipitationTotal, 1), unit: "mm" }, { icon: <Zap />, label: "Erosividade (R)", value: formatNumber(result.rainfallErosivity, 2) }, { icon: <Sprout />, label: "Fator K", value: formatNumber(k, 3) }, { icon: <Ruler />, label: "Comprimento (L)", value: formatNumber(slopeLength, 1), unit: "m" }, { icon: <Mountain />, label: "Declividade (S)", value: formatNumber(slopePercent, 1), unit: "%" }, { icon: <Calculator />, label: "Fator LS", value: formatNumber(result.topographicFactor, 3) }, { icon: <Leaf />, label: "Cobertura (CP)", value: formatNumber(cp, 3) }]; return <div className="eups-result-spotlight" aria-live="polite"><section className="eups-result-primary"><span className="eups-result-eyebrow">Resultado principal</span><strong className="eups-result-title">Perda média anual estimada</strong><div className="eups-result-total"><strong>{formatNumber(result.soilLoss, 2)}</strong><span>t/ha/ano</span></div><div className="eups-result-formula"><Calculator /><span>PS = K × R × LS × CP</span></div><div className={`eups-result-classification ${result.classification ? "is-complete" : "is-pending"}`}><span>Classificação</span><strong>{result.classification ?? "Pendente"}</strong></div></section><section className="eups-result-analysis"><h3>Resumo da análise</h3><div className="eups-result-factors">{factors.map((factor) => <ResultFactor key={factor.label} {...factor} />)}</div><div className={`eups-result-status ${result.isComplete ? "is-complete" : "is-pending"}`}><CheckCircle2 /><span>{result.isComplete ? "Todos os fatores informados" : "Cálculo pendente de informações"}</span></div></section></div>; }
function ResultFactor({ icon, label, value, unit }: { icon: ReactNode; label: string; value: string; unit?: string }) { return <article className="eups-result-factor"><span className="eups-result-factor-icon">{icon}</span><span><small>{label}</small><strong>{value}{unit ? ` ${unit}` : ""}</strong></span></article>; }
