...
export function EupsPage({ onModuleChange }: { onModuleChange: (module: GeoCalcModule) => void }) {
  ...
  useEffect(() => {
    ...
    setPreviewedRainfallStation(null);
  ...
  useEffect(() => {
    if (!isRainfallMapExpanded) return;
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape") setIsRainfallMapExpanded(false);
    };
  ...
  const selectRainfallStation = (station: InmetNormalStation | null) => {
    setPreviewedRainfallStation(null);
    if (!station) {
      setSelectedRainfallStationCode(null);
      return;
    }
  ...
  return <TooltipProvider><div className="app-layout">
    ...
    <main className="app-shell">
      ...
      <EupsLocationPanel
        latitudeText={rainfallLatitudeText}
        longitudeText={rainfallLongitudeText}
        coordinatesAreValid={rainfallCoordinatesAreValid}
        point={rainfallPoint}
        station={selectedRainfallStation}
        previewStation={previewedRainfallStation}
        hasManualAdjustments={rainfallHasManualAdjustments}
        onLatitudeChange={setRainfallLatitudeText}
        onLongitudeChange={setRainfallLongitudeText}
        onPointChange={updateRainfallPoint}
        onStationSelect={selectRainfallStation}
        onStationPreviewChange={setPreviewedRainfallStation}
...
function EupsLocationPanel({ latitudeText, longitudeText, coordinatesAreValid, point, station, previewStation, hasManualAdjustments, onLatitudeChange, onLongitudeChange, onPointChange, onStationSelect, onStationPreviewChange, onMapExpand }: { latitudeText: string; longitudeText: string; coordinatesAreValid: boolean; point: MapPoint | null; station: InmetNormalStation | null; previewStation: InmetNormalStation | null; hasManualAdjustments: boolean; onLatitudeChange: (value: string) => void; onLongitudeChange: (value: string) => void; onPointChange: (point: MapPoint) => void; onStationSelect: (station: InmetNormalStation | null) => void; onStationPreviewChange: (station: InmetNormalStation | null) => void; onMapExpand: () => void }) {
  ...
  return <section className="panel climate-panel eups-location-panel" aria-label="Local e dados observacionais"><PanelTitle icon={<MapPin className="size-4" />} title="Local e dados observacionais" description="Selecione uma estação INMET ou um ponto no mapa para preencher as precipitações mensais da EUPS." /><div className="climate-grid"><div className="climate-controls"><InmetStationCombobox period={EUPS_INMET_PERIOD} value={station} onChange={onStationSelect} onPreviewChange={onStationPreviewChange} /><div className="eups-location-coordinate-grid"><label className="eups-number-field" htmlFor="eups-rainfall-latitude"><span>Latitude <small>graus decimais</small></span><input id="eups-rainfall-latitude" aria-label="Latitude para estação INMET" inputMode="decimal" value={latitudeText} placeholder="Ex.: −15,7801" onChange={(event) => onLatitudeChange(event.target.value)} /></label><label className="eups-number-field" htmlFor="eups-rainfall-longitude"><span>Longitude <small>graus decimais</small></span><input id="eups-rainfall-longitude" aria-label="Longitude para estação INMET" inputMode="decimal" value={longitudeText} placeholder="Ex.: −47,9292" onChange={(event) => onLongitudeChange(event.target.value)} /></label></div>{coordinateMessage ? <p className={`eups-location-coordinate-message ${hasCoordinateText ? "is-error" : ""}`}>{coordinateMessage}</p> : null}<div className="inmet-period-card"><span>Período de referência</span><strong>1991–2020</strong><small>Normal climatológica oficial disponível para as estações exibidas.</small></div><div className="import-card inmet-source-card"><div><strong>Dados observacionais por estação</strong><span>Ao selecionar uma estação INMET completa, a tabela recebe automaticamente a precipitação mensal da normal climatológica 1991–2020.</span></div>{hasManualAdjustments ? <p className="eups-location-manual-adjustments">Valores ajustados manualmente após o preenchimento.</p> : null}</div></div><div className="eups-location-map"><MapPicker point={point} onPointChange={onPointChange} stations={listInmetStations(EUPS_INMET_PERIOD)} selectedStationCode={station?.code ?? null} previewStation={previewStation} onStationSelect={onStationSelect} /><button className="eups-map-expand-button" type="button" onClick={onMapExpand}><Maximize2 />Abrir mapa ampliado</button></div></div></section>;
}
function RainfallMapDialog({ isOpen, point, station, previewStation, onClose, onPointChange, onStationSelect }: { isOpen: boolean; point: MapPoint | null; station: InmetNormalStation | null; previewStation: InmetNormalStation | null; onClose: () => void; onPointChange: (point: MapPoint) => void; onStationSelect: (station: InmetNormalStation | null) => void }) {
  if (!isOpen) return null;
  return <div className="eups-map-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="eups-map-modal" role="dialog" aria-modal="true" aria-labelledby="eups-map-modal-title"><header className="eups-map-modal-header"><div className="eups-map-modal-heading"><span className="eups-result-eyebrow">Estações INMET</span><h2 id="eups-map-modal-title">Mapa de estações INMET</h2><p>Selecione um ponto ou uma estação para preencher a normal climatológica 1991–2020.</p></div><button className="eups-map-modal-close" type="button" aria-label="Fechar mapa ampliado" onClick={onClose} autoFocus><X /></button></header><MapPicker point={point} onPointChange={onPointChange} stations={listInmetStations(EUPS_INMET_PERIOD)} selectedStationCode={station?.code ?? null} previewStation={previewStation} onStationSelect={onStationSelect} /></section></div>;
}
