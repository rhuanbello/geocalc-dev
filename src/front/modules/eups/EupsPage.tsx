...
function ResultSpotlight({ soilLoss, classification }: { soilLoss: number | null; classification: ReturnType<typeof calculateEups>["classification"] }) { return <div className="eups-result-spotlight" aria-live="polite"><div className="eups-result-label"><span>Resultado principal</span><strong>Perda média anual estimada</strong></div><div className="eups-result-value"><strong>{formatNumber(soilLoss, 2)}</strong><span>t/ha/ano</span></div><div className={`eups-result-classification ${classification ? "is-complete" : "is-pending"}`}><span>Classificação</span><strong>{classification ?? "Pendente"}</strong></div></div>; }
function CalculationRow({ factor, value, unit, type, calculation, output = false }: { factor: string; value: string; unit: string; type: string; calculation: string; output?: boolean }) { return <tr className={output ? "eups-output-row" : "eups-input-row"}><th scope="row"><span className="eups-factor-code">{factor}</span></th><td className={output ? "output-cell" : "input-cell"}><strong>{value}</strong></td><td>{unit}</td><td><span className={`eups-type-tag ${output ? "output" : "input"}`}>{type}</span></td><td>{calculation}</td></tr>; }
